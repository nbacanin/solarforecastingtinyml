'''
funkcija za LSTM  model:
stavljaju se nekolko stacked LSTM layera
...
class LSTMFunction:
    ...
    def function(self, x):
        ...
        LSTMModel = self.createLSTMModel(self.features, self.lags, self.steps, nn_list,learning_rate=learning_rate)
    ...
    def createLSTMModel(self, feature_size, inputs, outputs, nn_list, dropout=0.01,
                                 recurrent_dropout=0.01,
                                 loss='mean_squared_error', optimizer='adam', activation='relu', metrics=['accuracy'],
        ...
        for i in range(len(nn_list)):
            ...
            #samo ako je prvi lstm sloj i ako imamo vise od jednog sloja, onda stavljamo batchsize i return sequences=true
                LSTMModel.add(layers.GRU(nn_list[i], batch_size=batch_size,activation=activation,recurrent_dropout=0, unroll=True,return_sequences=True))
            #ako je prvi lstm sloj i ako imamo samo jedan sloj, onda stavljamo batch_size, ali bez return_sequences
                ...
                LSTMModel.add(layers.GRU(nn_list[i], batch_size=batch_size, recurrent_dropout=0,unroll=True,activation=activation))

            #ako je poslednji lstm sloj, onda return_sequences=False
            elif i==len(nn_list)-1:
                LSTMModel.add(layers.GRU(nn_list[i], activation=activation,unroll=True,recurrent_dropout=0))
            else:
                LSTMModel.add(layers.GRU(nn_list[i], activation=activation, recurrent_dropout=0, unroll=True, return_sequences=True))
