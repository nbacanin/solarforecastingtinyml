...
class FA:
    ...
    def get_global_best(self):
        return (self.population[0].objective_function, self.population[0].overallResults, self.population[0].allResults,
                self.population[0].overallResults1, self.population[0].allResults1, self.population[0].results,
                self.population[0].model)

    def get_global_worst(self):
        return (
        self.population[-1].objective_function, self.population[-1].overallResults, self.population[-1].allResults,
        self.population[-1].overallResults1, self.population[-1].allResults1, self.population[-1].results,
        self.population[-1].model)
    ...
    def get_global_best_solution(self):
        ...
        for i in range(len(self.population)):
            indicator_list.append(self.population[
                                      i].overallResults[
                                      0])  # ovo je za indikator, u ovom slucaju R2, ali moze da se menja
